export async function load() {
	...
	const mainPostModules = import.meta.glob('$lib/posts/*.{md,svx}', { eager: true });
	const archivedPostModules = import.meta.glob('$lib/posts/archived/*.{md,svx}', { eager: true });

	// Combine both sets of posts
	const allPostModules = { ...mainPostModules, ...archivedPostModules };

	const posts = Object.entries(allPostModules).map(([filePath, module]) => {
		try {
			...
			if (fileName === 'we-built-this-blog-2.md') {
				...
				console.log('frontmatter.published === false:', frontmatter.published === false);
			}
			...
			try {
				...
				console.error(`Could not read frontmatter for ${fileName}:`, frontmatterError);
			}
